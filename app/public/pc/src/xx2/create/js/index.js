define(function(require, exports, module){
	...
	var INDEX = (function(){
		var
			$btnPagestatus = $('.J_btn_pagestatus'), // 顶部登录、领取奖励
			...
			// 已登录
			$login = $('#J_login'),
			$loginNickname = $('#J_login_nickname'),
			$logout = $('#J_logout'),
		...
		return {
					...
					if( d && d.status && d.status == 1 ){
						...
						if( !$.isEmptyObject(INFO.awards) ){
							...
							$('.J_popup_invite_get').on('click', function(e){
								var _this = $(this),
									_tid = _this.attr('data-id'),
									_ismask = _this.attr('data-mask');
			...
			bindEvent: function(){
				...
				$btnInvite.on('click', function(e){
					var _this = $(this),
						_yet = _this.closest('li').hasClass('yet');
