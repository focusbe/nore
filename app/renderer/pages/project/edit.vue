		...
		<div class="center">
			...
			<div class="page_detail">
				<div class="top_options_bar" v-if="curPage">
					<Button type="text" icon="md-trash" shape="circle" @click="clearCanvas" title="清空"></Button>
					<!-- <Button type="primary" @click="savePage" title="保存">
                        <Icon type="md-sync" size="20"/>
					</Button>-->
					<Button icon="md-code" shape="circle" @click="buildPage" type="text" title="生成HTML"></Button>
					<!-- `                 <Button @click="publishCode" type="primary" title="发布">
                        <Icon type="md-cloud-upload" size="20" color/>
					</Button>`-->
					<Button @click="showInFolder" icon="md-folder" shape="circle" type="text" title="在文件夹中显示"></Button>
					<my-psd
						:actname="actname"
						:pagename="curPage"
						:device="curdevice"
			...
			switch (event) {
				...
				case "curvnode":
					...
					if (!!params.vuenode && !!params.vuenode.elm) {
						this.$refs["editanimate"].changeCurElement(
							params.vuenode.elm
						);
					}
