 ...
 *
 */
define('vectorMap', function (require, exports, module) {
    ...
    (function ($) {
        ...
        VectorCanvas.prototype = {
            ...
            createText: function (params) {
                ...
                var tspanNode;
                if (this.mode == 'svg') {
                    var allCity = params.city;
                    for (var c in allCity) {
                        var xArea = allCity[c].x;
                        var yArea = allCity[c].y;
                        if (undefined != xArea && undefined != yArea && "" != xArea && "" != yArea) {
                            textNode[c] = this.createSvgNode('circle');
                            textNode[c].setAttribute('cx', xArea);
                            textNode[c].setAttribute('cy', yArea);
                            textNode[c].setAttribute('r', 1);
                            textNode[c].setAttribute('stroke', '#fff');
                            textNode[c].setAttribute('stroke-width', 0.4);
                            textNode[c].setAttribute('fill', '#902B2A');
                            textNode[c].setAttribute('data-cnname', allCity[c].cnname);
                            textNode[c].setAttribute('id', "text_" + allCity[c].enname);
        ...
        var WorldMap = function (params) {
            ...
            for (var key in mapData.pathes) {
                ...
                //为地图添加文字
                var param = { name: mapData.pathes[key].name, x: mapData.pathes[key].x, y: mapData.pathes[key].y, style: textStyle, city: mapData.pathes[key].city };
            ...
            $(params.container).delegate(this.canvas.mode == 'svg' ? 'path' : 'shape', 'mouseover mouseout', function (e) {
                var path = e.target, code = e.target.id.split('_').pop(), labelShowEvent = $.Event('labelShow.jvectormap'), regionMouseOverEvent = $.Event('regionMouseOver.jvectormap');
                ;
                if (e.type == 'mouseover') {
            ...
            params.container.mousemove(function (e) {
                if (map.label.is(':visible')) {
                    map.label.css({
                        left: (e.pageX - 15 - map.labelWidth) + 113,
                        top: (e.pageY - 15 - map.labelHeight) - 10
                    });
                }
            });
