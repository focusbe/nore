define(function (require, exports, module) {
    ...
    var doc = document;
    var POPUP = (function () {
        var $maskbg = $('#J_maskbg'), $popup = $('#J_popup'), $popupClose = $('.J_popup_close'), // 关闭按钮
        $popupWrap = $('.J_popup_wrap'), $popupSign = $('#J_popup_sign'), // 签到弹层
        ...
        return {
            init: function () {
                // M站判断
                if (UTILS.device().any())
                    win.location.href = './m/?fromid=' + (UTILS.getStr('fromid') || '');
                // 事件绑定
                this.bindEvent();
            },
            // 事件绑定
            bindEvent: function () {
                ...
                $btnSidebar.off().on('click', function (e) {
                    var _this = $(this), _parent = _this.closest('.sidebar'), _status = _parent.hasClass('close');
                    var w;
                    if (_status) {
                        w = 281;
