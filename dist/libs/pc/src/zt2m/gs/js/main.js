...
define(function (require, exports, module) {
    ...
    if (UTILS.device().any())
        win.location.href = './m';
    var MAIN = (function () {
        var $video = $('#video'), $btnShareWeibo = $('#btn_share_weibo'), // 微博分享
        $btnHomeurl = $('#btn_homeurl'), // 进入首页
        $btnSubscribe = $('#J_btn_subscribe'), // 我要猜巨星
        $page = $('.J_page'), // 页面
        $btnUnlock = $('.J_btn_unlock'), // 解锁按钮
        $audio = $('#audio'), $btnAudio = $('#J_btn_audio'), // 音频按钮
        $maskbg = $('#maskbg'), $popup = $('#popup'), $popupMsg = $('#popup_msg'), $popupClose = $('#J_popup_close'), $input_mob = $('#input_mob'), // 手机号
        $input_code_img = $('#input_code_img'), // 图片验证码
        $input_code_sms = $('#input_code_sms'), // 短信验证码
        $btn_code_img = $('#btn_code_img'), $btn_code_sms = $('#btn_code_sms'), $popup_btn_subscribe = $('#btn_subscribe'), setMobCode_flag = true, //防止获取验证码重复点击
        exp_mobile = /^1[3|4|5|7|8]\d{9}$/, popupMsgTimer = null, PT = '';
        return {
            init: function () {
                // video
                this.video();
            ...
            bindEvent: function () {
                ...
                $btn_code_sms.on('click', function (e) {
                    var _this = $(this), reClick = null, //定时器
                    reClick_time = 60, //秒
                    reClick_index = 1; //
                    checkMob();
                    if (!_checkmob)
                ...
                $btnUnlock.on('click', function (e) {
                    var _this = $(this), _i = _this.index(), _lock = _this.attr('data-lock');
                    stopAudio();
                    if (_lock) {
                        $maskbg.show();
                        $popupMsg.show();
                ...
                $btnAudio.on('click', function (e) {
                    var _this = $(this), _play = _this.attr('data-play');
                    if (_play) {
                        _t && clearInterval(_t);
            ...
            player: function (obj) {
                ...
                // 非IE内核浏览器
                    ...
                    if (obj['dom'].find('audio').length <= 0)
                        obj['dom'].append(strAudio);
                }
                return $('#' + playID);
            }
        };
